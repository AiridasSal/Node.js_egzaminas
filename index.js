...
app.get('/api/users', (req, res) => {
    MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true }, (err, client) => {
        if (err) {
            ...
            const db = client.db("nodejs_egzaminas");
            const collection = db.collection("users_db");
            collection.find({}, { projection: { _id: 0, id: 1, name: 1, email: 1, address: 1 } }).toArray((err, users) => {
                if (err) {
                    console.log(err);
...
app.get('/api/users/emails', (req, res) => {
    MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true }, (err, client) => {
        if (err) {
            ...
            const db = client.db("nodejs_egzaminas");
            const collection = db.collection("users_db");
            collection.find({}, { projection: { _id: 0, id: 1, name: 1, email: 1 } }).toArray((err, users) => {
                if (err) {
                    console.log(err);
                    res.status(500).send("Error fetching users from the collection");
                } else {
...
app.get('/api/users/address', (req, res) => {
    MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true }, (err, client) => {
        if (err) {
            ...
            const db = client.db("nodejs_egzaminas");
            const collection = db.collection("users_db");
            collection.find({}, { projection: { _id: 0, id: 1, name: 1, address: 1 } }).toArray((err, users) => {
                if (err) {
                    console.log(err);
                    res.status(500).send("Error fetching users from the collection");
                } else {
